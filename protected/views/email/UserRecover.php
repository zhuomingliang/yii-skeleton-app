...
<p>
You have requested us to recover your login credentials at <?php echo Html::encode(Yii::app()->name); ?> (<?php echo Html::link(Yii::app()->request->getBaseUrl(true), Yii::app()->request->getBaseUrl(true)) ?>).
</p>
...
<?php
$url = Yii::app()->request->getHostInfo().Html::normalizeUrl(array('user/update'));
echo Html::link('account settings', $url);
?>.
<?php } else { ?>
</p>
<p>
...
<?php
$url = Yii::app()->request->getHostInfo().Html::normalizeUrl(array('user/recoverPassword', 'id'=>$user->id, 'pass'=>$user->password));
